...
#include "eaqmaodv-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE("EaqmaodvRoutingProtocol");
...
const uint32_t RoutingProtocol::EAQMAODV_PORT = 654;
...
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::eaqmaodv::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("Eaqmaodv")
...
{
    static TypeId tid =
        TypeId("ns3::eaqmaodv::RoutingProtocol")
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == EAQMAODV_PORT)
                    {
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    socket->Bind(InetSocketAddress(iface.GetLocal(), EAQMAODV_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvSaqmaodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), EAQMAODV_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, EAQMAODV_PORT));
}
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), EAQMAODV_PORT));
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), EAQMAODV_PORT));
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, EAQMAODV_PORT));
}
                                              ...
                                              Ipv4Address origin)
{
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, EAQMAODV_PORT));
        }
    }
}
