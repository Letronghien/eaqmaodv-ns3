...
#   Tổng: 1420 runs
#
# *** CẦN BẠN RÀ LẠI TRƯỚC KHI CHẠY FULL ***
#   1) N=50 cho ELONG/STAT — theo lựa chọn bạn chọn (mật độ cao nhất family E).
#   2) 6 cấu hình ablation W1-W6 (biến ABLW bên dưới) — đây là ĐỀ XUẤT của tôi
#      dựa trên 2 fix chính (adaptive-alpha qua eaMu/eaKappa, energy^2 qua eaW3,
#      queue-reward qua eaW4) — CHƯA phải định nghĩa gốc của bạn, sửa lại nếu khác.
#   3) SIMTIME=200 mặc định theo CLI, nhưng notes có ghi chú "dùng 30s cho paper"
#      — kiểm tra lại giá trị thật bạn dùng cho paper rồi sửa biến SIMTIME bên dưới.
#
...
TEST_MODE=false
RESUME=false
SIMTIME=200   # <-- KIỂM TRA LẠI: notes ghi "dùng 30s cho paper", sửa nếu cần
...
generate_jobs() {
  ...
  if [[ "$RESUME" == true && -f "$JOBS_FILE" ]]; then
    ...
    echo -e "${Y}[RESUME]${N} Còn ${P} jobs pending, đã xong ${D}"
    return
  fi
  ...
  for NN in 10 20 30 40 50; do
    ...
    for PROTO in "${PROTOS[@]}"; do
      for RUN in $(seq 1 36); do
        echo "$PROTO $RUN $TAG --numNodes=$NN --mobility=GAUSS --scenario=default --simTime=$SIMTIME" >> "$JOBS_FILE"
        COUNT=$((COUNT+1))
      done
    done
  done

  # ── ELONG: Elongated topology, N=50 ──────────────────────────────────────
  for PROTO in "${PROTOS[@]}"; do
    for RUN in $(seq 1 30); do
      echo "$PROTO $RUN ELONG --numNodes=50 --mobility=GAUSS --scenario=elong --areaX=3000 --areaY=200 --simTime=$SIMTIME" >> "$JOBS_FILE"
      COUNT=$((COUNT+1))
    done
  done

  # ── STAT: Static nodes vel=0, N=50 ────────────────────────────────────────
  for PROTO in "${PROTOS[@]}"; do
    for RUN in $(seq 1 50); do
      echo "$PROTO $RUN STAT --numNodes=50 --mobility=GAUSS --scenario=stat --meanVelMin=0 --meanVelMax=0 --simTime=$SIMTIME" >> "$JOBS_FILE"
      COUNT=$((COUNT+1))
    done
  done

  # ── W: Ablation EA-QMAODV only, N=20 ── ĐỀ XUẤT, RÀ LẠI TRƯỚC KHI CHẠY ────
  ...
  for TAG in ablw1 ablw2 ablw3 ablw4 ablw5 ablw6; do
    for RUN in $(seq 1 20); do
      echo "EAQMAODV $RUN $TAG --numNodes=20 --mobility=GAUSS --scenario=$TAG ${ABLW[$TAG]} --simTime=$SIMTIME" >> "$JOBS_FILE"
      COUNT=$((COUNT+1))
    done
  done
...
while true; do
  JOB=\$(
    flock -x "\$LOCK" bash -c '
      ...
      [[ -n "\$LINE" ]] && sed -i "1d" "'\$JOBS_FILE'" && echo "\$LINE"
    '
  )
  [[ -z "\$JOB" ]] && break
  ...
  ./ns3 run "scratch/fanet-sim --protocol=\$PROTO --seed=\$RUN \$EXTRA --csvFile=\$WORKER_CSV" \
    > "\$LOG" 2>&1
  STATUS=\$?

  ELAPSED=\$(( \$(date +%s) - T0 ))

  # exit 0 = OK. exit 134/139 = SIGABRT/SIGSEGV lúc destructor EnergySourceContainer
  # (bug đã biết trước đây, CSV đã ghi xong TRƯỚC khi crash -> vẫn tính là thành công)
  if [[ \$STATUS -eq 0 || \$STATUS -eq 134 || \$STATUS -eq 139 ]]; then
